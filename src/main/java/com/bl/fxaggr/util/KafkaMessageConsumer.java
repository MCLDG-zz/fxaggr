...
import com.bl.fxaggr.generator.KafkaPriceEventGenerator;
/**
 * Simple Consumer
 */
public class KafkaMessageConsumer {
    private KafkaPriceEventGenerator priceEventGenerator = null;
    
  public KafkaMessageConsumer(KafkaPriceEventGenerator priceEventGenerator) {
      this.priceEventGenerator = priceEventGenerator;
    ...
    topicCountMap.put("pricefeed", 1);

    Map<String, List<KafkaStream<byte[], byte[]>>> consumerMap = consumerConnector.createMessageStreams(topicCountMap);

    List<KafkaStream<byte[], byte[]>> streamList = consumerMap.get("pricefeed");
    ...
    while(iterator.hasNext()) {
        ...
        priceEventGenerator.WriteToDisruptor(msg);
    }
  ...
  public static void processRecords(Map<String, ConsumerRecords<String, String>> records) {
    ...
    if(messages != null) {
      for (ConsumerRecord<String, String> next : messages) {
        try {
          System.out.println(next.value() + " - offset: " + next.offset());
        } catch (Exception e) {
          e.printStackTrace();
        }
      }
